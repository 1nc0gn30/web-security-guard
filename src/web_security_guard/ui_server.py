"""Google Security Studio UI Server & REST API.

Zero-dependency HTTP server delivering the Google Material 3 Security Studio,
...
class SecurityAuditEngine:
    ...
    def fetch_and_audit(cls, target_url: str) -> Dict[str, Any]:
        ...
        try:
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; WebSecurityGuard/1.0; +https://github.com/google/web-security-guard)"},
                method="HEAD"
            )
            # Try HEAD first
            try:
...
Generated by Google Security Studio (`web-security-guard`).
...
<html>
<head><title>Google Security Studio</title></head>
<body><h1>Google Security Studio</h1><p>Running Web Security Guard 1.0.0</p></body>
</html>"""
                self.send_response(HTTPStatus.OK)
                self.send_header("Content-Type", "text/html; charset=utf-8")
...
def start_ui_server(host: str = "127.0.0.1", port: int = 8080, public_dir: Optional[Path] = None, open_browser: bool = False) -> None:
    """Starts the Google Security Studio UI server."""
    server = create_server(host, port, public_dir)
    actual_port = server.server_port
    url = f"http://{host}:{actual_port}"
    print(f"🛡️ Google Security Studio UI running at {url}")
    ...
    try:
        ...
        print("\nShutting down Google Security Studio.")
    finally:
        server.server_close()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Google Security Studio UI Server")
